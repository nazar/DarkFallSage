class MobSpellRevision < ActiveRecord::Base
...
#  before_create :relink_after_revert, :if => :mob_reverting?
  ...
  def mob_reverting?
    MobSpell.revisable_current_states[:reverting] && (MobSpell.revisable_current_states[:reverting][self.revisable_original_id])
  end
