...
module Akismet
  ...
  def initialize
    ...
    @proxyHost = nil
    instance_eval do
      YAML::load(File.open(File.join(Rails.root,'config', 'akismet.yml'))).each do |k,v|
  ...
  def verify_akismet_key()
    ...
    logger.debug("resp: #{resp.to_yaml} data: #{data.to_yaml}")
  end
  ...
  def call_akismet(akismet_function, args)
    ...
    logger.debug("Akismet query to: #{http.to_yaml} with path:#{path} data: #{data.to_yaml}")
