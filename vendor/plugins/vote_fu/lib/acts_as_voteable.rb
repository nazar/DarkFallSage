    ...
    module Voteable #:nodoc:
      ...
      module ClassMethods
        def acts_as_voteable(options = {})
          ...
          options[:class]     ||= 'Voteable'
          #
          has_many :votes, :as => :voteable, :dependent => options[:dependant], :class_name => options[:class] 
      ...
      module InstanceMethods
        def votes_for
          Vote.count(:all, :conditions => [
            ...
            id, ActiveRecord::Base.send(:class_name_of_active_record_descendant, self.class).to_s, true
          ])
        end
        ...
        def votes_against
          Vote.count(:all, :conditions => [
            "voteable_id = ? AND voteable_type = ? AND vote = ?",
            id, ActiveRecord::Base.send(:class_name_of_active_record_descendant, self.class).to_s, false
          ])
        end
